...
// Supabase Edge Function — no API keys needed in frontend
...
export default function App() {
  ...
  const handleSync = async () => {
    setPhase('MATCHING');
    try {
      const response = await fetch(STYLE_ENDPOINT, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ request, identity, budget })
      });
      ...
      setResult({
        advice: "Simple shapes and clean contrast create a balanced aesthetic.",
        total: 49.98,
        items: [
  ...
  const reset = () => { setPhase('IDLE'); setRequest(''); setResult(null); };
    ...
    <div className="min-h-screen bg-white p-8 pt-20 animate-fade-up">
      ...
      <div className="max-w-md mx-auto flex flex-col items-center">
        ...
        <input type="range" min="50" max="2000" step="50" value={budget}
          onChange={(e) => setBudget(Number(e.target.value))} className="mb-24" />
    ...
    <div className="min-h-screen bg-white p-8 pt-20 animate-fade-up">
      ...
      <div className="max-w-md mx-auto flex flex-col items-center">
        <div className="w-full bg-[#F9F9F9] border border-transparent rounded-[32px] p-6 mb-12 focus-within:border-gray-200 transition-all">
          <textarea autoFocus value={request} onChange={(e) => setRequest(e.target.value)}
        ...
        <button onClick={handleSync} disabled={!request}
    ...
    <div className="min-h-screen bg-white p-8 pt-20 animate-fade-in pb-32">
      <div className="max-w-md mx-auto flex flex-col items-center">
        ...
        <div className="w-full bg-[#F9F9F9] p-10 rounded-[40px] mb-12 text-center border border-gray-100 shadow-sm">
          <p className="text-xl font-light italic leading-relaxed text-black/80 px-4">"{result?.advice}"</p>
        </div>
